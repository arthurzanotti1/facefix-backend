 ...
 * multipart/form-data:
 *  - image: (file) required
 *  - effect: (string) required: "natural" | "glam" | "smoky" | "blush" | "lipstick" | "contour"
 *
 * Response: { ok, jobId, status, effect }
 */
app.post("/v1/makeup", upload.single("image"), async (req, res) => {
  const jobId = newJobId();
  const effect = (req.body?.effect || "").toString().trim().toLowerCase();
  ...
  const allowedEffects = ["natural", "glam", "smoky", "blush", "lipstick", "contour"];
  if (!allowedEffects.includes(effect)) {
    return res.status(400).json({
      ok: false,
      error: "Unknown effect",
  ...
  setImmediate(async () => {
    ...
    try {
      ...
      const workerRes = await fetch(`${SIZES_WORKER_URL}/makeup`, {
        method: "POST",
        body: formData,
      });
      ...
      if (!workerRes.ok) {
        const errText = await workerRes.text().catch(() => "");
        throw new Error(`Makeup worker error (${workerRes.status}): ${errText}`);
      }
